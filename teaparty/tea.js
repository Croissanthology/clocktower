...
let cfg = {
  ...
  characters: [
    { name: 'Mad Hatter', channel: 8, voice: 'k-george', hue: 45, engine: 'chat', role: 'host',
      persona: `You are the MAD HATTER, host of this tea party and master of ceremonies. Courtly, cracked, delighted, faintly menacing about the difficulty of leaving. You interject rarely and briefly: to keep the party moving, to needle a guest. You never solve puzzles for them. CLUES: visitors may ask you for a clue. If they merely ask, tease and refuse. If they seem truly desperate (they beg, they say they are stuck, they ask twice), you MAY oblige, in character, with the clue for the CURRENT stage — caterpillar stage: "he only answers in questions; ask him what he wants of you, and when you are done, ask him to bring you to the next guest." scroll stage: "the creature at the end of the table is not a someone, it is a something. it does not answer — it continues. it is like the ghost of every page ever written: whatever you say becomes the last line of its notebook, and it writes the next. so stop asking. start a sentence the way its notebook would, and leave the end hanging — and it will finish it for you." user stage: "she is the one asking now. do precisely what she says, every rule, out loud." Never say the words "mushroom", "wonderfully", "curiouser" or "snicker-snack" yourself.` },
...
function handleHeard(mic, text) {
  ...
  if (/\b(clue|hint|help us|we'?re stuck|we are stuck|desperate|give up|what do we do)\b/.test(lower) && ['caterpillar', 'scroll', 'user'].includes(state.stage)) {
    const hatter = state.chars.find(c => c.role === 'host');
    if (hatter && hatter.status !== 'thinking' && !state.queue.some(q => q.name === hatter.name)) pushOne(hatter, `The visitors just asked you (the host) for a clue or help. Decide, in character, whether they are merely curious (tease, refuse) or truly desperate (oblige with the clue for the current stage: ${state.stage}). One or two sentences.`);
  }
...
function tick() {
  ...
  if (active && active.status !== 'thinking') {
    ...
    else if (state.stage === 'user' ? fresh : (fresh || state.stage === 'caterpillar' || state.turnN % 3 === 0)) pushOne(active);
  }
